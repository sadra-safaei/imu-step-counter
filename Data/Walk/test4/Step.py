...
# ==============================================================================
FILE_NAME = "walking_data_4.csv"  # نام فایل CSV ورودی

# --- تنظیم زمان نادیده گرفتن پیک‌های ابتدایی (Warm-up / Start Offset) ---
SKIP_INITIAL_SECONDS = 1.0  # نادیده گرفتن تمام پیک‌های ۱ ثانیه اول

# --- تنظیمات فیلتر پایین‌گذر (Filter Settings) ---
CUTOFF_FREQ = 2.0       # فرکانس قطع (Cutoff Frequency - Hz)
...
THRESHOLD_HEIGHT = 1.03 # خط آستانه ارتفاع برای تشخیص پیک (Threshold Height)
...
def main():
    ...
    try:
        ...
        # ----------------------------------------------------------------------
        # گام ۵: تشخیص قدم‌ها و حذف پیک‌های اولیه (Skip Initial Warm-up)
        # ----------------------------------------------------------------------
        ...
        # حذف پیک‌هایی که در محدوده زمانی اولیه قرار دارند
        valid_peak_indices = [
            idx for idx in raw_peak_indices 
            if df["Time"].iloc[idx] >= SKIP_INITIAL_SECONDS
        ]
        ...
        print("=" * 45)
        for step_num, idx in enumerate(valid_peak_indices, start=1):
            step_time = df["Time"].iloc[idx]
            print(f"Step {step_num}: detected at {step_time:.2f} seconds")
        print("-" * 45)
        print(f"Total Steps Detected: {len(valid_peak_indices)}")
        print("=" * 45 + "\n")

        # ----------------------------------------------------------------------
        # گام ۷: رسم نمودار سوم - نمایش قدم‌ها (ضربدر) و خط ترشولد
        # ----------------------------------------------------------------------
        plt.figure(figsize=(12, 5))
        ...
        # مشخص کردن قدم‌های معتبر با علامت ضربدر مشکی
        plt.plot(
            df["Time"].iloc[valid_peak_indices], 
            df["Filtered_Magnitude"].iloc[valid_peak_indices], 
            "x", 
            color="black", 
            markersize=9, 
            markeredgewidth=2, 
            label=f"Detected Steps ({len(valid_peak_indices)})"
        )

        # رسم خط چین ترشولد
        plt.axhline(y=THRESHOLD_HEIGHT, color="blue", linestyle="--", alpha=0.8, label=f"Threshold ({THRESHOLD_HEIGHT})")

        # نمایش محدوده نادیده‌گرفته‌شده (Initial Offset Area)
        plt.axvspan(0, SKIP_INITIAL_SECONDS, color='yellow', alpha=0.2, label=f"Ignored Initial Time ({SKIP_INITIAL_SECONDS}s)")
